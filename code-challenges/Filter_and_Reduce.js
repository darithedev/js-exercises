...
function sumNumbers(arr){
  const total = 0;
  const sum = arr.reduce((tot, value) => tot + value, total);

  return sum;
}

console.log(sumNumbers([3,3,3,3,8])) // 20
console.log(sumNumbers([2,2,2,2,2])) // 10
...
console.log()
console.log("-------- TASK 3 --------");
const uniqueCategories = menuItems.reduce((unique, category) => {
  if (!unique.includes(category.category)) {
    unique.push(category.category)
  }
  return unique;
}, []);
...
console.log()
console.log("-------- TASK 4 --------");
const neighborhoods = coffeeShops.map(n => n.neighborhood);
const uniqueNeighborhoods = [...new Set(neighborhoods)];

const neighborhoodObject = {};

uniqueNeighborhoods.forEach(neighborhood => {
  neighborhoodObject[neighborhood] = coffeeShops.filter(n => n.neighborhood === neighborhood).length;
});
